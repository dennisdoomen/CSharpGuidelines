...
- Always put opening and closing curly braces on a new line.
  ...
  **Exception:** Simple properties/events without a body, such as:
  ```csharp
...
- Remove redundant parentheses in expressions if they do not clarify precedence. Add parentheses in expressions to avoid non-obvious precedence:
  ```csharp
  overruled || (enabled && active)   // nested conditional
  foo | (bar >> size)                // bitwise and shift
  ```
...
Maintaining a common order allows other team members to find their way in your code more easily. In general, a source file should be readable from top to bottom, as if reading a book.
